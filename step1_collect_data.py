...
global url,fnum,bnum,rnum,lnum,snum,cdir
cdir=os.getcwd()
os.mkdir('testsdc')
fnum=0
snum=0
...
class window(QtWidgets.QMainWindow):
   def __init__(self):
      ...
      self.setWindowTitle("Self Driving Car App | rahul vinod")
   ...
   def forward(self):
      ...
      io.imsave(r'.\testsdc\forward_'+str(fnum)+".jpg",img)
      self.image.setPixmap(QtGui.QPixmap(r'.\testsdc\forward_'+str(fnum)+".jpg"))
      fnum=fnum+1
   ...
   def right(self):
      ...
      self.s.write(b'R')
      img=io.imread(self.url)
      io.imsave(r'.\testsdc\right_'+str(rnum)+".jpg",img)
      self.image.setPixmap(QtGui.QPixmap(r'.\testsdc\right_'+str(rnum)+".jpg"))
      rnum=rnum+1
   ...
   def left(self):
      ...
      self.s.write(b'L')
      img=io.imread(self.url)
      io.imsave(r'.\testsdc\left_'+str(lnum)+".jpg",img)
      self.image.setPixmap(QtGui.QPixmap(r'.\testsdc\left_'+str(lnum)+".jpg"))
      lnum=lnum+1
   ...
   def backward(self):
      ...
      self.s.write(b'B')
      img=io.imread(self.url)
      io.imsave(r'.\testsdc\backward_'+str(bnum)+".jpg",img)
      self.image.setPixmap(QtGui.QPixmap(r".\testsdc\backward_"+str(bnum)+".jpg"))
      bnum=bnum+1
   ...
   def stopdir(self):
      ...
      self.s.write(b's')
      img=io.imread(self.url)
      io.imsave(r'.\testsdc\stop_'+str(snum)+".jpg",img)
      self.image.setPixmap(QtGui.QPixmap(r'.\testsdc\stop_'+str(snum)+".jpg"))
      snum=snum+1
